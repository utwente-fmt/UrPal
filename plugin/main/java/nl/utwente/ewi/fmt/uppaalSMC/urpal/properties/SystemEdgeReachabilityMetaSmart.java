...
@SanityCheck(name = "System Edge Reachability meta smart")
        ...
        try {
            ...
            engineQuery(tSys, q, OPTIONS, (qr, ts) -> {
                ...
                if (qr.getStatus() == QueryResult.OK || qr.getStatus() == QueryResult.MAYBE_OK) {
                    sys.getProcesses().stream().flatMap(e -> e.getEdges().stream()).map(SystemEdge::getEdge).distinct()
                    ...
                    try {
                        ...
                        allEdges.forEach(e -> e.setProperty("color",
