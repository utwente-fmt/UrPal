...
@SanityCheck(name = "Template edge Reachability meta", description = "")
public class TemplateEdgeReachabilityMeta extends AbstractProperty {
	...
	public void doCheck(NSTA nstaOrig, Document doc, UppaalSystem sys, Consumer<SanityCheckResult> cb) {
		...
		nsta.getTemplate().forEach(eTemplate -> {
			...
			eTemplate.getEdge().forEach(e -> {				
				...
				id.getIndex().add(UppaalUtil.createLiteral("" + (offset.get() + edgeList.indexOf(e))));
			...
			offset.addAndGet(eTemplate.getEdge().size());
		});

		index.setSizeExpression(UppaalUtil.createLiteral("" + templateEdges.size()));
		((ValueIndex) varMeta.getIndex().get(0)).setSizeExpression(UppaalUtil.createLiteral("" + templateEdges.size()));
		
		String q = "E<>(forall (i : int[0, " + (templateEdges.size() - 1) + "]) _f[i])";
		try {
				...
				if (qr.getStatus() == QueryResult.OK || qr.getStatus() == QueryResult.MAYBE_OK) {
					...
					cb.accept(new SanityCheckResult() {
						...
						public JPanel toPanel() {
							...
							p.add(new JLabel("All edge reachable!"));
							return p;
						}
					});
				} else {
